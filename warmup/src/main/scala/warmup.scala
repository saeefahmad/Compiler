...
object warmup {
  ...
  def dup(e: Expr):Expr = e match {
    ...
    case Times(Num(2), e2) => Plus(dup(dup(e2)), dup(e2)) // 2 in the left: adding two substring instead
    case Times(e1, Num(2)) => Plus(dup(e1), dup(e1)) // 2 in the right: adding two substring instead
    case Times(e1, e2) => Times(dup(e1), dup(e2)) // default match if two is not in any multiplying subexpression
    case Num(value)    => Num(value)
  }
  ...
  def main(argv: Array[String]) {
    val e : Expr = Times(Num(2), Plus(Num(1),Times(Num(3),Num(2)))) // test case
